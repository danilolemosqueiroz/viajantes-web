...
export default function ModalLogin({
  ...
  aberto: boolean;
  aoFechar: () => void;
  /** Nome do atrativo (ou do plano) na faixa de cima; sem ele, não há faixa. */
  nome?: string;
  capa?: string | null;
  titulo: string;
  ...
  const comFaixa = nome !== undefined;

  const botaoFechar = (classes: string) => (
    <button
      type="button"
      onClick={fechar}
      aria-label={t('Fechar')}
      className={`flex size-9 shrink-0 items-center justify-center rounded-pilula transition ${classes}`}
    >
      <X size={18} aria-hidden="true" />
    </button>
  );

  /** Título da tela e, sem a faixa, o botão de fechar ao lado dele. */
  const cabecalho = (tituloDaTela: string, abaixo: React.ReactNode) => (
    <div className="flex items-start justify-between gap-3">
      ...
      {!comFaixa && botaoFechar('-mr-2 -mt-1 text-texto-2 hover:bg-brand-suave hover:text-brand')}
    </div>
  );
      ...
      <div className="modal-folha__rolagem">
        {comFaixa && (
          <div className="relative h-32 overflow-hidden bg-brand sm:h-36">
            ...
            <div className="absolute right-3 top-3">{botaoFechar('bg-white/90 text-brand shadow-flutua hover:bg-white')}</div>
          </div>
        )}
        ...
        <div className="space-y-4 px-5 pb-5 pt-4 sm:px-6">
          {tela === 'entrar' ? (
            <>
              {cabecalho(
                ...
                <>
                  ...
                  <p className="mt-1 text-mini text-texto-3">{t('É grátis e usa a mesma conta do aplicativo.')}</p>
                </>,
              )}
            ...
            <>
              {cabecalho(
                t('Criar conta'),
                <p className="mt-1 text-nota text-texto-2">{t('Vale no site e no aplicativo, com a mesma conta.')}</p>,
              )}
